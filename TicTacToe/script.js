class TicTacToe {
    ...
    setupEventListeners() {
        ...
        document.getElementById('difficulty').addEventListener('change', (e) => {
            ...
            this.handleDifficultyChange(newDifficulty);
        });
    }
    
    handleDifficultyChange(newDifficulty) {
        // Check if user has been playing (has scores > 0)
        const hasExistingScores = this.scores.player > 0 || this.scores.ai > 0 || this.scores.draws > 0;
        
        if (hasExistingScores) {
            // Show confirmation dialog
            const confirmed = confirm(
                `You have existing scores (You: ${this.scores.player}, AI: ${this.scores.ai}, Draws: ${this.scores.draws}).\n\n` +
                `Do you want to erase the scores and switch to ${newDifficulty.charAt(0).toUpperCase() + newDifficulty.slice(1)} difficulty?\n\n` +
                `Click OK to proceed or Cancel to keep current difficulty.`
            );
            
            if (confirmed) {
                // User confirmed - reset scores and change difficulty
                this.scores = {
                    player: 0,
                    ai: 0,
                    draws: 0
                ...
                // User cancelled - revert the select element to previous value
                document.getElementById('difficulty').value = this.difficulty;
            }
        } else {
            // No existing scores, just change difficulty
            this.difficulty = newDifficulty;
            this.updateStatus();
        }
    }
    ...
    updateScores(result) {
        switch (result) {
            ...
            case 'draw':
                ...
                break;
            case '':
                // Just update the display without changing scores
                break;
        }
